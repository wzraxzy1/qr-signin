...
- **防重复签到**：
  - **复合键判重**：姓名 + 手机号 + 工号 三者完全相同视为同一人重复签到
  - **强唯一字段**：工号、身份证号、学号 三个字段中，**任一字段值已存在**即判为重复签到（两条规则同时生效）
- **最多签到人数**：创建/编辑会话时可设置签到人数上限（可选，留空=不限制）；达到上限后自动拒绝新签到，手机端显示「签到人数已满」
...
1. **创建会话**：在管理面板点击「创建会话」
   - 填写会话名称（如"周一晨会签到"）
   - 选择二维码刷新间隔（5/10/15秒）
   - 配置签到字段（字段名、标签、类型、是否必填）；**默认已包含：姓名、手机号、工号、身份证号、学号**（后三项默认非必填，可手动开启必填）
   - 设置签到时间窗口（开始/停止时间，可选）
   - 设置最多签到人数（可选，留空=不限制）
...
- **免费版（Free）**：SQLite 数据存在临时磁盘上，每次部署或实例休眠唤醒后数据会被重置。适合临时演示和测试。
- **付费版（Starter $7/月）**：已挂载 1GB 持久磁盘（`render.yaml` 中 `disk` 配置，挂载到 `/opt/render/data`），后端通过 `RENDER_DATA_DIR` 环境变量把 `signin.db` 写到该磁盘，**部署/重启后数据保留**。

> ⚠️ 升级到付费档并首次启用磁盘时，旧容器上的数据不会被迁移（新磁盘初始为空），这一次要重新创建会话；之后数据永久保留。
